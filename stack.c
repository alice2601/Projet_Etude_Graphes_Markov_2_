...
stack creer_stack(void) {
    return NULL;
}

// renvoie vrai si la pile est vide
bool stack_vide(stack p) {
    return (p == NULL);
}

// empile un élément (ajoute en tête)
stack empiler(stack p, t_tarjan_vertex v) {
...
stack depiler(stack p, t_tarjan_vertex* v) {
    if (p == NULL) {
